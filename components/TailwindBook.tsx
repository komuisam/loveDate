    ...
    >
      ...
      <div
        className={cn(
          ...
          isMobile
            ...
            : "rounded-r-md border-y-[8px] border-r-[8px] border-l-[4px]",
        )}
        style={{
          backfaceVisibility: "hidden",
          borderColor: color,
          borderLeftColor: isMobile ? color : "transparent",
      ...
      <div
        className={cn(
          ...
          isMobile
            ? "rounded-md border-[8px]"
            : "rounded-l-md border-y-[8px] border-l-[8px] border-r-[4px]",
        )}
        style={{
          backfaceVisibility: "hidden",
          transform: "rotateY(180deg)",
          borderColor: color,
          borderRightColor: isMobile ? color : "transparent",
          backgroundClip: "padding-box",
        }}
      >
        {backContent}
...
export function TailwindBook({
  coverColor,
  dataPage,
  setDataPage,
  onSelect,
}: BookProps) {
  const { addSavedDate, savedDates, bookSheetIndex: currentSheetIndex, setBookSheetIndex } = useStore();
  const setCurrentSheetIndex = React.useCallback((val: number | ((prev: number) => number)) => {
    const next = typeof val === 'function' ? val(currentSheetIndex) : val;
    setBookSheetIndex(next);
  }, [currentSheetIndex, setBookSheetIndex]);

  const ideas = useMemo(() => Object.values(dateIdeas), []);
  ...
  const sheets = useMemo(() => {
    ...
    arr.push({
      id: "cover",
      front: { type: "cover", data: null },
      back: isMobile ? null : { type: "idea", data: ideas[0] },
    });
    ...
    if (isMobile) {
      ...
      while (i < ideas.length) {
        arr.push({
          id: `idea-${i}`,
          front: { type: "idea", data: ideas[i] },
          back: { type: "idea", data: ideas[i + 1] || null },
        });
        i += 2;
      }
    }
  ...
  const [pageInput, setPageInput] = useState(String(isMobile ? currentSheetIndex + 1 : currentSheetIndex + 2));
  ...
  const applyPageJump = (value: string) => {
  ...
  const handlePageJump = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter" || e.key === "Escape") {
      ...
      if (e.key === "Enter") applyPageJump(pageInput);
  ...
  return (
    <div className="w-full flex flex-col items-center gap-6 py-8">
      ...
      <div
        className={cn(
          "relative h-[500px]",
          isMobile ? "w-full max-w-sm px-4" : "w-[340px] md:w-full max-w-5xl",
        )}
        style={{ perspective: "1500px" }}
      >
        {!isMobile && (
          <div
            className="absolute left-1/2 top-0 bottom-0 w-4 -ml-2 rounded-sm"
        ...
        <div
          className={cn(
            "absolute top-0 h-full preserve-3d transition-all duration-500",
                ...
                <div className="h-full flex flex-col items-center justify-between text-center w-full">
                  {sheet.front.type === "cover" ? (
                    <div className="flex-1 flex flex-col items-center justify-center">
                      ...
                      <div
                        className="w-16 h-1 bg-current mb-4 rounded-full"
                    ...
                    <>
                      <div className="w-full flex justify-between items-center text-xs text-stone-400 font-serif uppercase tracking-widest border-b pb-2">
                        ...
                        <span>#{sheet.front.data?.id}</span>
                      </div>
                      <div className="flex-1 flex flex-col items-center justify-center py-4">
                        <h2 className="text-2xl font-medium text-stone-700 font-serif mb-4 leading-snug">
                          {sheet.front.data?.title}
                        </h2>
                        <div className="w-12 h-12 rounded-full flex items-center justify-center bg-stone-50 text-stone-300 mb-4">
                          <span className="text-xl font-serif">
                            {sheet.front.data?.id}
                          </span>
                        </div>
                        <p className="text-sm text-stone-500 line-clamp-6">
                          Una cita romántica perfecta...
                        </p>
                      </div>
                      <div className="w-full pt-4 border-t">
                        <Button
                          variant="outline"
                          size="sm"
                          className="w-full text-xs hover:bg-stone-50"
                          onClick={(e) => {
                            e.stopPropagation();
                            openSelectionModal(sheet.front.data);
                          }}
                        >
                          Seleccionar Idea
                        </Button>
                  ...
                  {sheet.back ? (
                    <>
                      {sheet.back.type === "idea" && sheet.back.data ? (
                        <>
                          <div className="w-full flex justify-between items-center text-xs text-stone-400 font-serif uppercase tracking-widest border-b pb-2">
                            <span>#{sheet.back.data.id}</span>
                            <span>Date Night</span>
                          </div>
                          <div className="flex-1 flex flex-col items-center justify-center py-4">
                            <h2 className="text-2xl font-medium text-stone-700 font-serif mb-4 leading-snug">
                              {sheet.back.data.title}
                            </h2>
                            <div className="w-12 h-12 rounded-full flex items-center justify-center bg-stone-50 text-stone-300 mb-4">
                              <span className="text-xl font-serif">
                                {sheet.back.data.id}
                          ...
                          <div className="w-full pt-4 border-t">
                            <Button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                openSelectionModal(sheet.back?.data);
                              }}
                            >
                              Seleccionar Idea
                            </Button>
                        ...
                        <div className="flex-1 flex items-center justify-center">
                          <div className="w-16 h-16 rounded-full bg-stone-100/50" />
                        </div>
                      )}
                    </>
                  ) : (
      ...
      <div className="flex items-center gap-4 mt-4">
        ...
        <span className="text-sm text-stone-500 font-serif flex items-center gap-1">
          {isMobile ? "Idea" : "Página"}
          <input
            type="number"
            value={pageInput}
            ...
            onBlur={() => {
              if (debounceRef.current) clearTimeout(debounceRef.current);
              applyPageJump(pageInput);
              setIsEditingPage(false);
            }}
            className="w-10 text-center font-medium text-stone-800 border-b border-stone-300 focus:border-stone-600 bg-transparent outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
          />
          / {isMobile ? ideas.length : sheets.length + 1}
        </span>
        <Button
          variant="outline"
          size="icon"
